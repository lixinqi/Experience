"""
tmux_send_ctrl_backward :=
    FutureTensor
    <- $grad_output FutureTensor
    # inline

Backward for ft_tmux_send_ctrl.
...
def tmux_send_ctrl_backward(ctx, grad_output) -> FutureTensor:
    """Backward for ft_tmux_send_ctrl: reconstruct attrs + GradFn."""
    ...
    dispatch = get_backward_dispatcher(tmux_send_ctrl_backward)
    if dispatch({}):
        return grad_output
    return TmuxSendCtrlGradFn.apply(grad_output)
