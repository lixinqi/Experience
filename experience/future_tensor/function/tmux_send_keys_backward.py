...
Backward for ft_tmux_send_keys.
...
from experience.future_tensor.function.tmux_send_keys_2nd import TmuxSendKeysGradFn
from experience.future_tensor.backward_dispatch.backward_dispatcher import get_backward_dispatcher


def tmux_send_keys_backward(ctx, grad_output) -> FutureTensor:
    """Backward for ft_tmux_send_keys: reconstruct attrs + GradFn."""
    ...
    dispatch = get_backward_dispatcher(tmux_send_keys_backward)
    if dispatch({}):
        return grad_output
    return TmuxSendKeysGradFn.apply(grad_output)
