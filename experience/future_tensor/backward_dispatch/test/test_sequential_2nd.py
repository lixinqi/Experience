...
Groups:
  ...
  2. Natural 2nd-derivative flow dispatches sequential_backward record
...
class SequentialOnlyModel(nn.Module):
    """Minimal model containing only ft_sequential."""

    def forward(self, *inputs):
        return ft_sequential(*inputs)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ft_a = make_overflow_ft([3], ["a0", "a1", "a2"], tmpdir)
    ft_b = make_overflow_ft([3], ["b0", "b1", "b2"], tmpdir)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ft_a = make_overflow_ft([2], ["a0", "a1"], tmpdir)
    ft_b = make_overflow_ft([2], ["b0", "b1"], tmpdir)
    ft_b.requires_grad_(True)
    ...
    if len(coll) >= 1:
        run_test("record fn is sequential_backward", coll[0].fn is sequential_backward)
        run_test("record has grad_output",
                 "grad_output" in coll[0].inputs)
        run_test("record has num_inputs",
                 "num_inputs" in coll[0].inputs)
        run_test("record num_inputs value is 2",
                 coll[0].inputs.get("num_inputs") == 2)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    if len(coll) >= 1:
        rec = coll[0]
        run_test("record.fn is sequential_backward", rec.fn is sequential_backward)
        ...
        run_test("num_inputs in inputs", "num_inputs" in rec.inputs)


# ── Summary ───────────────────────────────────────────────────────────────────
print(f"\n  Passed: {passed}, Failed: {failed}, Total: {passed + failed}")
if failed == 0:
    print("All ft_sequential 2nd-derivative tests passed.")
else:
    sys.exit(1)
